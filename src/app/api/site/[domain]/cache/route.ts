...
import { scanCache } from '@/lib/cachecheck';
...
// On-demand cache & compression check for ONE page (headers of the page + a sample of its assets).
// Defaults to the homepage; optional ?url= must be on the same site. App-side only.
export async function GET(req: Request, { params }: { params: Promise<{ domain: string }> }) {
  ...
  try {
    return NextResponse.json(await scanCache(target));
  } catch (e) {
